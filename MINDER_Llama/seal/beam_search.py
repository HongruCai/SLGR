...
class IndexBasedLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
            ...
            decoder_prefix_len:int = 1 #llama add
        ):
        self.index = index
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if self.forced_bos_token_id is not None:
            if input_ids.size(1) == self.decoder_prefix_len:
        ...
        if input_ids.size(1) == self.decoder_prefix_len:
            ...
            for batch_id, beam_sent in enumerate(input_ids_list):
                for beam_id, sent in enumerate(beam_sent):
                    ...
                    if sent[-1] in (self.eos_token_id, self.pad_token_id):
                        ...
                        low, high = self.index.get_range(self.force_decoding_from + sent[self.decoder_prefix_len:])
                        count = self.index.get_count(self.force_decoding_from + sent[self.decoder_prefix_len:-1])

                    else:
                        low, high = self.index.get_range(sent[self.decoder_prefix_len:])
                        count = self.index.get_count(sent[self.decoder_prefix_len:-1])
            ...
            for batch_id, beam_sent in enumerate(input_ids_list):
                for beam_id, sent in enumerate(beam_sent):
                    ...
                    distinct = torch.LongTensor(distinct).to(scores.device)
                    # print(distinct)
                    # print(test_tokenizer.batch_decode(distinct, skip_special_tokens=True, clean_up_tokenization_spaces=False))
...
def constrained_beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            ####  'LlamaForCausalLM' object has no attribute 'adjust_logits_during_generation'
...
def fm_index_generate(
    ...
    **kwargs,
    ...
    model.generation_config.min_length=min_length
    ...
    if not disable_fm_index:
        constrained_decoding_processor = IndexBasedLogitsProcessor(
                ...
                decoder_prefix_len=input_ids.size(1)
            )
    ...
    model_kwargs = model.generation_config.update(**kwargs)
    ...
    input_ids, model_kwargs = model._expand_inputs_for_generation(
        input_ids=input_ids,
        expand_size=model.generation_config.num_beams,
