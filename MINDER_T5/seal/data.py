...
class SlgrQueryQasIterator(QueryIterator):

    def get_query(self, id_):
        return self.topics[id_]['question'], self.topics[id_]['answers']
    ...
    def __iter__(self):
        for id_ in self.order:
            question, answers = self.get_query(id_)
            yield id_, question, answers
...
def get_query_iterator(topics_path: str, topics_format: TopicsFormat, queries_path: Optional[str] = None):
    mapping = {
        ...
        TopicsFormat.SLGR: SlgrQueryQasIterator,
    }
