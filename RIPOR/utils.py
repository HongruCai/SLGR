...
class Trie(object):
    def __init__(self, sequences: List[List[int]] = []):
        ...
        self.len = 0
        if sequences:
            for sequence in tqdm(sequences, desc="Building Trie"):
...
class T5Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels = " ".join(f"c_{c}" for c in ids[1:])
...
class LTRTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        ltr_loss = self.re_multi_ltr_loss(model, inputs, logits, labels)

        loss = self.ltr_loss_factor * ltr_loss + tem_loss

        if self.total % 1000 == 0:
            print(self.ratio, "/", self.total)
        return (loss, outputs) if return_outputs else loss
    ...
    def re_multi_ltr_loss(self, model, inputs, logits, labels):
