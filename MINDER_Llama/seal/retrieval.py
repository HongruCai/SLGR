...
from transformers import AutoConfig, AutoModelForSeq2SeqLM, AutoTokenizer, BartTokenizer, BartForConditionalGeneration,AutoModelForCausalLM,AutoTokenizer
    ...
    def process_batch(inputs):
        ...
        if searcher.decode_body:
            ...
            decoded_body = fm_index_generate(
                ...
                **batch, 
                min_length=prefix_len_query+20,
                max_length=prefix_len_query+20,
...
class SEALSearcher:
    ...
    def __init__(
        ...
        **params):
        ...
        if 'bart' in self.backbone or 'Llama' in self.backbone or 'llama' in self.backbone:
            ...
            self.title_eos_token_id = 29992

            self.query_bos_token = '||'
            self.query_bos_token_id = 3830
            self.query_eos_token = '@@'
            self.query_eos_token_id = 29992

            self.code_bos_token = '@@'
            self.code_bos_token_id = 29992
            self.code_eos_token = '||'
            self.code_eos_token_id = 3830
            self.prepend_space = True
            self.strip_token_ids = (0, 2)

            # self.title_bos_token = '</s>'
            # self.title_bos_token_id = 2
            # self.title_eos_token = '<|title_eos|>'
            # self.title_eos_token_id = 32000

            # self.query_bos_token = '<|query_bos|>' 
            # self.query_bos_token_id = 32001
            # self.query_eos_token = '<|query_eos|>'
            # self.query_eos_token_id = 32002

            # self.code_bos_token = '<|code_bos|>'
            # self.code_bos_token_id = 32003
            # self.code_eos_token = '<|code_eos|>'
            # self.code_eos_token_id = 32004
            # self.prepend_space = False
            # self.strip_token_ids = (1, 2)
            # self.special_token_ids_list = [
            #         2,         # self.title_bos_token_id
            #         32000,     # self.title_eos_token_id
            #         32001,     # self.query_bos_token_id
            #         32002,     # self.query_eos_token_id
            #         32003,     # self.code_bos_token_id
            #         32004,     # self.code_eos_token_id
            #         32005,     # "###"
            #         32006      # pad_token_id
            #     ]
            logger.log(logging.WARN, f"Using Llama's special tokens")
    ...
    def load_bart(bart_model_path: str, device: str = "cpu", backbone="facebook/bart-large", fairseq_checkpoint=True):
        ...
        base_path = backbone
        # base_path = './output/llama/7B'

        import torch
        if torch.cuda.is_available() and torch.cuda.is_bf16_supported():
            logger.log(logging.WARN, "BF16 supported, using BF16")
            precision =  torch.bfloat16
        else:
            logger.log(logging.WARN, "BF16 not supported, falling back to FP16")
        ...
        model.to(device)
        logger.log(logging.WARN, f"base model successfully loaded from {base_path}")
    ...
    def load(cls, fm_index_path, bart_model_path, device="cpu", **params):
        ...
        searcher = cls(
            ...
            bart_tokenizer, 
            bart_model, 
            bart_scorer_model=bart_model,
            bart_title_model=bart_model,
            bart_code_model=bart_model, 
            **params
        )
        return searcher
