...
from transformers import AutoConfig, AutoModelForSeq2SeqLM, AutoTokenizer, BartTokenizer, BartForConditionalGeneration, T5Tokenizer, T5ForConditionalGeneration
import torch
import random
...
class SEALSearcher:
    ...
    def __init__(
        ...
        **params):
        ...
        #     self.strip_token_ids = (0, 1)
            ...
            logger.log(logging.WARN, f"Using T5 backbone and setting tokens")
            self.title_bos_token = '</s>'
            self.title_bos_token_id = 1
            self.title_eos_token = '<extra_id_99>'
            self.title_eos_token_id = 32000

            self.query_bos_token = '<extra_id_97>'
            self.query_bos_token_id = 32002
            self.query_eos_token = '<extra_id_99>'
            self.query_eos_token_id = 32000
    ...
    def load(cls, fm_index_path, bart_model_path, device="cpu", **params):
        ...
        # )
        # bart_tokenizer = T5Tokenizer.from_pretrained(params.get('backbone', "google-t5/t5-base"))
        bart_tokenizer = T5Tokenizer.from_pretrained("google-t5/t5-small")
        if torch.cuda.is_available() and torch.cuda.is_bf16_supported():
            ...
            dtype = torch.float16
        bart_model = T5ForConditionalGeneration.from_pretrained(bart_model_path,
                                                                torch_dtype=dtype)
        ...
        # print('load model on',device)
        logger.log(logging.WARN, bart_model_path +" successfully loaded")
...
#                            keys[k] = (k, self.fm_index.get_count(list(k)))
    ...
    def batch_search_slgr(self, queries, ans, k: int = 10, added_documents=None, detokenize=None) -> List[List[SEALDocument]]:
        topk=k
        if detokenize is None:
            detokenize = self.detokenize
        retrieved = []
        ...
        results, keys = zip(*self.batch_retrieve_from_keys_slgr(keys, ans))
        # print('keys:',keys)
...
#                            keys[k] = (k, self.fm_index.get_count(list(k)))
    ...
    def retrieve_from_keys_slgr(self, keys, ans):
        ...
        a = ans[0]
        # print('a:',a)
        tokenized_a = self.bart_tokenizer(' '+a)
        # print('tokenized_a:',tokenized_a)
        pos_rg = self.fm_index.get_range(tokenized_a['input_ids'][:-1])
        # print('pos_row:',pos_rg)
        # for pos_row in range(pos_rg[0],pos_rg[1]):
        pos_tkn = self.fm_index.locate(pos_rg[0])
        # print('pos_tkn:',pos_tkn)
        pos_doc = self.fm_index.get_doc_index(pos_tkn)
        # print('pos_doc:',pos_doc)
        pos_docs.add(pos_doc)

        # doc_content = self.fm_index.get_doc(pos_doc)
        # print('doc_content:', self.bart_tokenizer.decode(doc_content))
        # sys.exit()
        total_docs = 21015324  
        num_neg_samples = 256

        all_docs = set(range(total_docs))
        available_neg_docs = list(all_docs - pos_docs)

        neg_docs = random.sample(available_neg_docs, num_neg_samples)

        final_docs = list(pos_docs) + neg_docs
        results, ngrams = rk.aggregate_evidence_slgr(
            specified_docs=final_docs,
            ngrams_and_scores=keys,
            unigram_scores=unigram_scores,
            index=self.fm_index,
            max_occurrences_1=self.max_hits,
    ...
    def batch_retrieve_from_keys_slgr(self, keys, ans):
        if self.jobs >= 2:
            yield from self._mp_batch_retrieve_from_keys_slgr(keys,ans)
        else:
            yield from self._batch_retrieve_from_keys_slgr(keys,ans)
    ...
    def _mp_batch_retrieve_from_keys_slgr(self, keys, ans):
        assert self.jobs >= 2
        idx = id(self)
        setattr(sys.modules['__main__'], f'_searcher_global_{idx}', self)
        with multiprocessing.Pool(self.jobs) as pool:
            task_list = zip(keys, ans)
            for i, (res, ngrams) in enumerate(pool.imap(_retrieve_from_keys_mp_aux_slgr, tqdm.tqdm(
                [(idx, kk, aa) for kk, aa in task_list],
                desc="Retrieving from keys",
                disable=not self.progress
            ))):
                if self.print_n_doc:
    ...
    def _batch_retrieve_from_keys_slgr(self, keys, ans):
        keys = tqdm.tqdm(
                zip(keys, ans),
                desc="Retrieving from keys",
                disable=not self.progress
            )
        for i, (kk, aa) in enumerate(keys):
            if self.print_n_doc:
                print(i)
            yield self.retrieve_from_keys_slgr(kk, aa)
...
def _retrieve_from_keys_mp_aux_slgr(args):
    idx, keys, ans = args
    return getattr(sys.modules['__main__'], f'_searcher_global_{idx}').retrieve_from_keys_slgr(keys,ans)
